...
# some changes
# redownload from https://github.com/iheanyi/bandcamp-dl and w/e 'cause there's finally a bugfix for single tracks
...
class BandcampDownloader():
    ...
    def template_to_path(self, track):
        path = self.template
        path = path.replace("%{artist}", unicode(track['artist']))
        path = path.replace("%{album}", unicode(track['album']))
...
class Bandcamp:
    ...
    def extract_album_meta_data(self, request):
        ...
        album['title'] = '' if not 'album_title' in embedData['EmbedData'] else embedData['EmbedData']['album_title']
        album['artist'] = stringBlock['TralbumData']['artist']
        album['tracks'] = stringBlock['TralbumData']['trackinfo']
        album['date'] = "" if stringBlock['TralbumData']['album_release_date'] == "null" else stringBlock['TralbumData']['album_release_date'].split()[2]
