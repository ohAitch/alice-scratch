...
;
(defn write [imgoid name] (write-png (if (instance? BufferedImage imgoid) imgoid ‹‹imgoid .reback› .img›) (str "pics/"name".png")))
...
(defn imgs-write-std [& imgs] (let [r (rand-nth (range 100 1000))] (dotimes [i (len imgs)] (write-std (nth imgs i) (str "debug"r"-"i)))))
(defn rgb-dist [a b]
...
(def-pattern pointing-at-iron-rocks {:is-text true})
...
(def task-mine-falador-iron
  #(let [t (mouse-loc)] ;! current: work on this
    ...
    (sleep 150)
    (print "ummmmm" ‹ik-pointing-at-iron-rocks .matches-now world›) ;! AAAAARGH they screw with the fonts >.<
    (mouse-move t)
    ))
