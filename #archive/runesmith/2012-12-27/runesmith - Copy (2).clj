...
(deftype Image [size
  ...
  (invoke [this x y] (aget pixels ‹‹y * X› + x›))
  (applyTo [this args] (clojure.lang.AFn/applyToHelper this args))
  (seq [this] (seq pixels))
  
  (s [this x y v] (aset-int pixels ‹‹y * X› + x› v))
  (draw-square [this x y rgb radi]
    (doseq [x' (range ‹‹x - radi› + 1› ‹x + radi›)
            y' (range ‹‹y - radi› + 1› ‹y + radi›)]
      ‹this .s x' y' rgb›)))
(defn mk-image [^java.awt.image.BufferedImage bi] ;! i _want_ a real ctor
  (let [sx ‹bi .getWidth›
        sy ‹bi .getHeight›
        backing bi
    ...
    (Image. [sx sy] sx sy pixels backing)))
(defn write-image [img path] (write-png ‹img .backing› path))
