...
def build_comment_data(view, pt):
    ...
    for suffix in suffixes:
        ...
        if start:
            ...
            if startstrip[-1] == "↩": startstrip = startstrip[:-1]

        if start and end:
            block_comments.append((start, end, disable_indent == 'yes'))
            block_comments.append((startstrip, end.strip(), disable_indent == 'yes'))
        elif start:
            line_comments.append((start, disable_indent == 'yes'))
            line_comments.append((startstrip, disable_indent == 'yes'))
