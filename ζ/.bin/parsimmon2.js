...
var P = (ι,...ιs)=>0?0
	...
	: _interrobang_('cant make parser from',ι)
...
var alt = P.alt = (...ps)=>{ ps = ps.map(P.X); ps["‖"] || _interrobang_(); return __alt(ps) }
...
var seq_map = P.seq_map = (...a)=>{ var f = a[-1]; a = a.slice(0,-1); Tfun(f) || _interrobang_(); return P(a).map(ι=> f(...ι)) }
