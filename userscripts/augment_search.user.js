...
// @require     file:///~/ali/scratch/remember.js
// @grant       none
// ==/UserScript==
...
var data = []; remember_data.filter(function(v){return !/^$|^\/\//.test(v)}).forEach(function(v){v = v.split(' '); v.slice(0,-1).forEach(function(t){data.push([t.split('&'), v[v.length-1]])})}); data.reverse()
...
if ($('#searchform').length && $('input[title="Search"]').length) {
	...
	setInterval(function(){if ($('input[title="Search"]').val() !== input) {input = $('input[title="Search"]').val(); update()}},50)
}
