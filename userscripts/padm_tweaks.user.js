...
if (location.href.match(/expost.padm.us/)) {
	...
	if (t=location.href.match(/^(.+)\?useMonospaceFont=true$/)) window.location = t[1]
		...
		;[0.8,1,1.2,1.4,1.6,1.8,2,2.2,2.4,2.6,2.8,3,3.5,4,5,6,7,8,9,10,12,14,16,18,20,25,30,35,40,45,50,55,60].map(function(v){setTimeout(function(){window.pad.changeViewOption('useMonospaceFont',true)},v*1000)})
		var confirm_ = window.confirm; window.confirm = function(v){return v==='Clear authorship colors on entire document?' || confirm_(v)}
	}
}
