...
var tokenize = function(s) {{var r = [];
	...
	var spos = function(v) {{if (!(v["SPACE"])) (v["pos"] = pos)
	else "";
	return(v)}};
	var increment_pos = function(s) {{seq(s)["map"](function(v) {{if ((v==="\n")) (pos = [(pos[0]+1),1,1])
	...
	(s = s["slice"](1))}};
	var end_any = function() {{r["push"](spos(S(any)));
	increment_pos(any);
	(any = undefined)}};
...
var groups_expand = function(tokens) {{var groups = {"(":")",
	...
	return(r[0])}}
var φ_f = function(in_,out) {{fs["writeFileSync"]((((out+"/")+in_["match"]((/([^\/]+)\.[^\/]+$/))[1])+".η"),JSON["stringify"](groups_expand(tokenize((fs["readFileSync"](in_)+"//! wth")))[0]))}}
